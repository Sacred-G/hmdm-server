...
/**
 * <p>A listener for the events of {@link CustomerCreatedEvent} type. This listener is responsible for saving the new customer settings to DB.</p>
 *
 * @author seva
 */
public class CustomerCreatedEventListener implements EventListener<CustomerCreatedEvent> {
    ...
    public void onEvent(CustomerCreatedEvent event) {
        DeviceInfoPluginSettings settings =
                settingsDAO.getPluginSettings(event.getCustomer().getId());
        if (settings == null) {
