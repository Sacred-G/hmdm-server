...
public interface PushScheduleMapper {
    ...
    @Insert("INSERT INTO plugin_push_schedule "
            ...
            + "min, minBit, hour, hourBit, day, dayBit, weekday, weekdayBit, month, monthBit) " + "VALUES "
            + "(#{customerId}, #{deviceId}, #{groupId}, #{configurationId}, #{scope}, #{messageType}, #{payload}, #{comment}, "
    ...
    @SelectKey(
            statement = "SELECT currval('plugin_push_schedule_id_seq')",
            keyColumn = "id",
            keyProperty = "id",
            before = false,
    ...
    @Update("UPDATE plugin_push_schedule "
            ...
            + "month=#{month}, monthBit=CAST(#{monthBit} AS BIT(12)) " + "WHERE id=#{id} AND customerId=#{customerId}")
    ...
    List<PluginPushSchedule> findMatchingTime(
            @Param("minMask") String minMask,
