...
public class ConfigurationDAO extends AbstractLinkedDAO<Configuration, Application> {
    ...
    public boolean hasConfigurationAccess(int configurationId) {
        Configuration c = getSingleRecordWithCurrentUser(currentUser -> this.mapper
                .checkConfigurationAccess(currentUser.getCustomerId(), currentUser.getId(), configurationId));
        return c != null;
    }
    ...
    public void insertConfiguration(Configuration config) {
        insertRecord(config, configuration -> {
            ...
            if (files != null && !files.isEmpty()) {
                files.forEach(file -> {
                    if (Boolean.FALSE.equals(file.getOverrideDevicePath())) {
    ...
    public void updateConfiguration(Configuration config) {
        updateRecord(config, configuration -> {
            ...
            // #6159: When updating the configuration via configuration editor page the state of Kiosk Mode flag is
            // specified explicitly by the submitted form value, so there is no need to re-check the state of that flag
            // based on presence of content app in configuration
                        ...
                        .forEach(file -> {
                            try {
                                ...
                                if (legacyFile != null
                                    ...
                                    final String checksum = CryptoUtil.calculateChecksum(URI.create(file.getExternalUrl()).toURL().openStream());
            ...
            /*
             * final List<Integer> filesToRemove = config.getFilesToRemove(); if (filesToRemove != null) { filesToRemove.forEach(fileId -> {
    ...
    public List<Application> getConfigurationApplications(Integer id) {
        return getLinkedList(
                id,
                this.mapper::getConfigurationById, customerId -> this.mapper
                        .getConfigurationApplications(customerId, id, "ca" + CryptoUtil.randomHexString(8)),
                SecurityException::onConfigurationAccessViolation);
    }
    ...
    /**
     * <p>
     * Upgrades the useage of specified application by specified configuration to most recent version available for application.
     * </p>
     *
    ...
    public void upgradeConfigurationApplication(Integer configurationId, Integer applicationId) {
        updateLinkedData(configurationId, this.mapper::getConfigurationById, configuration -> {
