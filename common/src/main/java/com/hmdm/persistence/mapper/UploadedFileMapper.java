...
public interface UploadedFileMapper {
    ...
    @Insert("INSERT INTO uploadedFiles (customerId, filePath, description, uploadTime, devicePath, "
            ...
            + "#{external}, #{externalUrl}, #{replaceVariables})")
    @SelectKey(
            statement = "SELECT currval('uploadedfiles_id_seq')",
            keyColumn = "id",
            keyProperty = "id",
            before = false,
    ...
    @Select("SELECT * FROM uploadedFiles " + "WHERE customerId = #{customerId} " + "ORDER BY filePath")
    ...
    @Select("SELECT * FROM uploadedFiles "
            ...
            + "ORDER BY filePath")
    List<UploadedFile> getAllByValue(@Param("customerId") int customerId, @Param("value") String value);
    ...
    @Select("SELECT * FROM uploadedFiles WHERE customerId = #{customerId} AND "
            ...
            + "    devicePath IS NOT DISTINCT FROM #{devicePath} AND " + "    external=#{external} AND "
            + "    externalUrl IS NOT DISTINCT FROM #{externalUrl} AND " + "    replaceVariables=#{replaceVariables}")
    UploadedFile findSame(
    ...
    @Select("SELECT COUNT(*) AS cnt " + "FROM uploadedFiles "
            + "WHERE NOT external AND filePath=#{filePath} AND id != #{id} AND customerId=#{customerId}")
    long countUploadedDuplicates(
            @Param("id") Integer id, @Param("customerId") Integer customerId, @Param("filePath") String filePath);

    @Select("SELECT COUNT(*) AS cnt " + "FROM uploadedFiles "
            + "WHERE external AND externalUrl=#{externalUrl} AND id != #{id} AND customerId=#{customerId}")
    long countExternalDuplicates(
            @Param("id") Integer id, @Param("customerId") Integer customerId, @Param("externalUrl") String externalUrl);
}
